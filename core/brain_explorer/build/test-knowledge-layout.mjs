...
const knowledgeSource = await readFile(
    new URL("../src/presentation/components/knowledge-view.ts", import.meta.url),
    "utf8"
);
...
const structureTreeSource = await readFile(
    new URL("../src/presentation/components/structure-tree.ts", import.meta.url),
    "utf8"
);
const viewsStyles = await readFile(new URL("../src/styles/views.css", import.meta.url), "utf8");

assert.match(knowledgeSource, /#scope = "all"/);
assert.match(knowledgeSource, /new Set\(\["global", "local"\]\)/);
assert.match(knowledgeSource, /"Global knowledge"[\s\S]*"Local knowledge"/);
assert.match(knowledgeSource, /"memory", "Global memory", memoryEntries[\s\S]*"pictures", "Pictures"/);
assert.match(knowledgeSource, /"memory", "Local memory", \[\][\s\S]*"logs", "Logs"[\s\S]*"messages", "Messages"/);
assert.match(knowledgeSource, /\.filter\(root => this\.#selectedScopes\.has\(root\.scope\)\)/);
assert.match(knowledgeSource, /data-filter-kind='kg-scope'[\s\S]*change", \(\) => this\.#applyFilters\(\)/);
assert.match(knowledgeSource, /#classSuperDomain\(scope\)[\s\S]*visualType: "class"/);
assert.match(knowledgeSource, /sortKey: `\$\{\(\{ memory: 0/);
...
assert.match(knowledgeSource, /#pictureTreeEntry\(picture\)[\s\S]*alreadyPrefixed[\s\S]*sourcePrefixes:/);
...
assert.match(knowledgeSource, /#animateViewport\(target, onComplete = null\)[\s\S]*onComplete\?\.\(\)/);
...
assert.match(knowledgeSource, /#positionComponent\(component, adjacency, byId, offsetX, offsetY, footprints\)[\s\S]*rowsPerColumn[\s\S]*columnGap[\s\S]*layerGap/);
assert.match(knowledgeSource, /#componentBounds\(component, byId, footprints\)[\s\S]*footprint\.width[\s\S]*footprint\.height/);
assert.match(knowledgeSource, /#layoutDomainGrid\(nodes, startY, footprints\)[\s\S]*columnWidth[\s\S]*cellHeight/);
...
assert.match(knowledgeSource, /#drawNodeLabel\(context, node, selected, ranked = false, occupiedBounds = \[\]\)[\s\S]*context\.measureText\(label\)[\s\S]*#roundedRect/);
...
assert.match(knowledgeSource, /#rankedLabelPlacement\(node, width, height, occupiedBounds\)[\s\S]*const candidates = \[[\s\S]*const overlaps[\s\S]*occupiedBounds\.push/);
...
assert.match(knowledgeSource, /#clearGraphFocus\(\)[\s\S]*#resetGraphRegion\(\)[\s\S]*#needsViewportFit = true[\s\S]*#drawCanvas\(\)/);
...
assert.match(knowledgeSource, /domainParts = parts\.slice\(memoryIndex \+ 1\)[\s\S]*replace\(\/\\\.\[\^\.\]\+\$\//);
