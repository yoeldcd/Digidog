"""Fixed JavaScript quality gates over the dedicated ESTree summary."""
...
from ...shared.gate_ids import JAVASCRIPT_GATE_IDS
from ...shared.protocol import BaseLanguageAnalyzer
from .parser import JavaScriptParser, JavaScriptParseResult
...
_JS_DOC_BLOCK = re.compile(r"/\*\*([\s\S]*?)\*/")
_JS_DOC_TAG = re.compile(r"@([A-Za-z][A-Za-z0-9_-]*)(?:\s+([^@\n*]+))?")
...
def _documentation_block(
    ...
    Args:
        source: Complete JavaScript or JSX source text.
    ...
    for match in _JS_DOC_BLOCK.finditer(source):
    ...
    for match in _JS_DOC_TAG.finditer(body):
...
def _parameter_names(source: str, line: int, name: str) -> tuple[str, ...]:
    """Extract callable parameter names from a bounded declaration signature.
...
def _section_present(sections: tuple[str, ...], required: str) -> bool:
    """Match section aliases while preserving policy spelling in evidence.
...
def _documentation_enabled(policy: LanguageQualityPolicy) -> bool:
    """Return whether any JavaScript documentation rule is active.

    Args:
        policy: Complete JavaScript quality policy.
...
def _documentation_evidence(
    ...
    Args:
        ...
        path: Safe relative artifact path.
        digest: SHA-256 digest for the complete source.
        source: Complete JavaScript or JSX source text held in memory.
    ...
    for declaration in parsed.declarations:
        ...
        is_class = "Class" in declaration.kind
        is_callable = "Function" in declaration.kind or declaration.kind in {
            "MethodDefinition",
            "TSDeclareFunction",
            "TSDeclareMethod",
            "TSMethodSignature",
        ...
        if requires_documentation and not documented:
            findings.append(
                _evidence(
                    ...
                    digest,
                    declaration.line,
                    "class JSDoc is required" if is_class else "callable JSDoc is required",
                )
            )
...
def _token_matches(kind: str, token: str) -> bool:
    """Match policy token aliases to ESTree node kinds.
...
def _vertical_evidence(
    ...
    """Check configured blank lines around declaration and clause nodes.
...
def _syntax_gates(
    parsed: JavaScriptParseResult, path: str, digest: str
    ...
    return tuple(
        ...
        for gate_id in JAVASCRIPT_GATE_IDS
    )


class JavaScriptAnalyzer(BaseLanguageAnalyzer):
    """Analyze JavaScript and JSX with exactly four fixed gates."""

    language = Language.JAVASCRIPT
    gate_ids = JAVASCRIPT_GATE_IDS

    def __init__(self, parser: JavaScriptParser | None = None) -> None:
    ...
    def _analyze(
        ...
        Args:
            artifact: In-memory JavaScript artifact.
            policy: Complete JavaScript quality policy.
        ...
        if syntax_gates is not None:
            ...
            gates = (
                _gate("JS-SYNTAX", [], "syntax valid", policy),
                _gate(
                    "JS-DOCUMENTATION",
                    documentation,
                ...
                _gate("JS-COMPACTNESS", compactness, "compactness valid", policy),
            )
...
def evaluate_javascript(
    content: str, policy: LanguageQualityPolicy, path: str = "artifact.js"
) -> tuple[GateResult, ...]:
    """Evaluate JavaScript source and return the exact four language gates.

    Args:
        content: Complete JavaScript source held in memory.
        policy: Complete JavaScript quality policy.
    ...
    try:
        ...
        safe_path = "artifact.js"
    artifact = InMemoryFile(
        path=safe_path, language=Language.JAVASCRIPT, content=content
    )

    return JavaScriptAnalyzer().analyze(artifact, policy).gates


__all__ = ["JavaScriptAnalyzer", "evaluate_javascript"]
