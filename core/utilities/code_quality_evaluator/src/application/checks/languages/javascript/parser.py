"""In-memory JavaScript/JSX parsing backed by the pinned ESTree subprocess."""
...
class DeclarationSummary:
    """Describe one JavaScript or JSX declaration.
...
class LayoutNodeSummary:
    """Describe a syntax node used by vertical-layout checks.
...
class JavaScriptParseResult:
    """Immutable JavaScript parse and source-structure summary.
...
class JavaScriptParser:
    """Parse JavaScript and JSX source without touching the filesystem."""
    ...
    def parse(self, source: str) -> JavaScriptParseResult:
        """Parse source and return immutable syntax/declaration/comment summaries.

        Args:
            source: Complete JavaScript or JSX source held in memory.

        Returns:
            JavaScriptParseResult: Typed parser and source-structure summary.
        """
        outcome = self._runner.run_javascript(source, timeout=self._timeout)

        return self._result(outcome, source)

    @staticmethod
    def _result(outcome: NodeParserOutcome, source: str = "") -> JavaScriptParseResult:
        ...
        Returns:
            JavaScriptParseResult: Immutable parsed summary or syntax failure.
        """
...
_METHOD_PATTERN = re.compile(
    r"^\s*(?:(?:public|private|protected|static|abstract|async|override|readonly|get|set)\s+)*"
...
def _augment_method_declarations(
    parsed: JavaScriptParseResult, source: str
    ...
    Returns:
        JavaScriptParseResult: Parsed result with source-proven method summaries.
    """
...
def parse_javascript(
    ...
    """Parse JavaScript or JSX source through a dedicated parser instance.
    ...
    Returns:
        JavaScriptParseResult: Immutable parser summary.
    """

    return JavaScriptParser(runner=runner).parse(source)


JavaScriptParseSummary = JavaScriptParseResult
