...
def test_reply_composer_uses_frameless_translucent_avatar_chrome() -> None:
    ...
    from brain.presentation.avatar.qt.runtime.window import QtAvatarWindow, reply_composer_geometry
    ...
    try:
        ...
        screen = avatar.app.screenAt(bubble_geometry.center()) or avatar.app.primaryScreen()
...
def test_database_without_workspace_configuration_fails_before_cwd_write() -> None:
    with TemporaryDirectory() as directory:
        ...
        contaminated_path = working_directory / "$agent" / "database" / "avatar_communication.db"
