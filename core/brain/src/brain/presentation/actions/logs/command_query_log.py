...
from brain.application.profiles.service import render_profile_template_value
...
def _localize_match(
    ...
    """Return a localized copy of one public query match."""
    localized = render_profile_template_value(match, workspace_root)
    if not isinstance(localized, dict):
        raise TypeError("Localized query match must remain a dictionary.")
    return {k: v for k, v in localized.items() if k not in ("path", "source_path", "file_path", "filepath")}
...
def handle(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if domain:
            ...
            domain = resolve_query_log_domain(
                domain, list_log_domains(workspace_root=workspace_root)
            )
        try:
            ...
            matches = manager.search_logs(
                query_str, domain_filter=domain, limit=args.limit
            )
            ...
            if (
                not is_embedding_unavailable_error(search_error)
                and "embedding" not in str(search_error).casefold()
            ):
                raise
        ...
        log_matches = matches
        log_matches = [_localize_match(match, workspace_root) for match in log_matches]
        policies = _live_policy_matches()
        policies = [_localize_match(match, workspace_root) for match in policies]
        ...
        if args.json:
            ...
            print(
                render_placeholders(
                    f"# Workspace log query: '__CYAN__{query_str}__RESET__'",
                    color_enabled,
                )
            )
            if policies:
                print(
                    render_placeholders(
                        "## __GREEN__Active workspace policies__RESET__", color_enabled
                    )
                )
            if not log_matches:
                print(
                    render_placeholders(
                        "__YELLOW__No matching log entries found.__RESET__",
                        color_enabled,
                    )
                )
            elif not policies:
                print(
                    render_placeholders(
                        "## __GREEN__Matching log entries__RESET__", color_enabled
                    )
                )
            print()
