...
def handle(args: argparse.Namespace) -> int:
    """Write Markdown content to a memory-domain entry.

    Args:
        args (argparse.Namespace): Parsed command options with the target domain,
            entry key, content, and output settings.

    Returns:
        int: Zero when the entry is saved; otherwise one after reporting a
