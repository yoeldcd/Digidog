...
def _print_detail(args: argparse.Namespace, message: str) -> None:
    ...
    Args:
        args (argparse.Namespace): Parsed command options controlling verbose activity.
        message (str): Detail message text.
...
def _print_progress(args: argparse.Namespace, message: str = "") -> None:
    ...
    Args:
        ...
        message: Optional progress message text.
...
def _print_object(args: argparse.Namespace, label: str, identifier: str, detail: str = "") -> None:
    ...
    Args:
        ...
        identifier: Primary object identifier.
...
def _memory_source_paths() -> list[str]:
    ...
    Returns:
        list[str]: Active relative memory markdown source paths.
    """
...
def _memory_registry_records() -> list[object]:
    ...
    Returns:
        list[object]: Active memory source registry records.
    """
...
def _log_source_paths(workspace_root: Path) -> tuple[list[str], list[str]]:
    ...
    Returns:
        tuple[list[str], list[str]]: Canonical and legacy log relative paths.
    """
...
def handle(args: argparse.Namespace) -> int:
    """Initialize workspace stores, refresh indexes, and render session context.

    Args:
        args (argparse.Namespace): Parsed initialization options controlling output
            format, activity logging, and optional maintenance behavior.

    Returns:
        int: Zero when initialization completes; otherwise one after reporting an
            error.
    """
    ...
    logs_database = workspace_root / "$agent" / "database" / "brain_logs.db"
    _print_object(args, "logs database", logs_database.as_posix())
    ...
    if migration_report.actions:
        _print_progress(
            args,
            render_placeholders(
                f"__GREEN__Runtime store migrations: {len(migration_report.actions)} action(s).__RESET__",
                color_enabled,
            ),
        )
        ...
        _print_progress(
            args,
            render_placeholders(
                "__DIM__Runtime stores already use current database layout.__RESET__",
                color_enabled,
            ),
        )
    ...
    _print_detail(
        args,
        (
            "memory scan covered __CYAN__{sources}__RESET__ markdown files across "
            "__CYAN__{domains}__RESET__ top-level domains."
        ).format(
    ...
    _print_detail(
        args,
        (
            "memory registry tracks __CYAN__{sources}__RESET__ active files with "
            "__GREEN__{entries}__RESET__ detected entries."
        ).format(
    ...
    _print_detail(
        args,
        (
            "found __CYAN__{canonical}__RESET__ canonical logs and "
            "__YELLOW__{legacy}__RESET__ legacy candidates / {total} files."
        ).format(
    ...
    _print_detail(
        args,
        (
            "backlog migration imported __GREEN__{imported}__RESET__ task(s); "
            "__CYAN__{existing}__RESET__ persisted task(s) kept unchanged."
        ).format(
    ...
    _print_detail(
        args,
        (
            "logs DB tracks __GREEN__{entries}__RESET__ entries across "
            "__CYAN__{domains}__RESET__ domains with "
            "__CYAN__{latest}__RESET__ latest-index rows."
        ).format(
            entries=entry_count,
