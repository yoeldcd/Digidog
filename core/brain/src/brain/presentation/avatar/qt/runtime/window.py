...
from brain.presentation.avatar.qt.backlog.application.composition import create_backlog_window
...
from brain.presentation.avatar.qt.avatar.geometry import (
    QtWindowGeometryMixin, bubble_position, bubble_vertical_lane,
    clamp_bubble_position, reply_composer_geometry,
)
from brain.presentation.avatar.interactivity.quota_view_model import quota_reset_label as _shared_quota_reset_label
...
class QtAvatarWindow(
    ...
    def __init__(self, start_polling: bool = True) -> None:
        ...
        self.controls = QtAvatarControls(
            self, self._activate_message_control, lambda: self._post("/mute"), self._toggle_pin,
            self._avatar_click, self._refresh_quotas, self._toggle_last_message,
            lambda: self._post("/cancel-processing"), self._open_backlog_window,
    ...
    def _backlog_window_for_action(self) -> BacklogWindow:
        """Return the singleton backlog coordinator without changing its visibility."""
    ...
    def _open_capture_form(self) -> None:
        """Open or reuse the capture form without showing the task-list window."""
        backlog = self._backlog_window_for_action()
        backlog.hide()
        backlog.open_capture_form()

    def _signal_window_ready(self) -> None:
        """Signal completed GUI setup; process registration alone is not readiness."""
