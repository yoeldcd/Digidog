...
def resolve_enabled_stage_config(
    ...
    Args:
        ...
        stage_name: `str`. The identifier of the stage to resolve.
        source_path: `str`. The file system path used for diagnostic event reporting.
        event_callback: `LLMEventCallback | None`. An optional callback for emitting stage-related events.

    Returns:
        StageModelConfigDTO: Enabled stage configuration.

    Raises:
        KnowledgeLLMError: Raised if the specified stage is not found or is explicitly disabled.
    """
...
def resolve_stage_api_key(
    ...
    Args:
        ...
        event_callback (LLMEventCallback | None): Optional event sink.
        stage_name: `str`. The identifier of the stage for diagnostic reporting.
        source_path: `str`. The file system path used for diagnostic event reporting.
        stage_config: `StageModelConfigDTO`. The configuration DTO containing the API key reference.
        event_callback: `LLMEventCallback | None`. An optional callback for emitting stage-related events.
    ...
    Raises:
        KnowledgeLLMError: Raised if the API key cannot be resolved from the configuration or environment.
    """
