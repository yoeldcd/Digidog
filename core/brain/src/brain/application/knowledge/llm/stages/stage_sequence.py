...
from brain.application.knowledge.llm.stages.stage_runner import generate_delta_with_llm
...
def generate_multistage_deltas(
    ...
    Args:
        ...
        source_path: `str`. Stable source path for the content being analyzed.
        content: `str`. The raw source text to be processed by the LLM stages.
        base_delta: `KnowledgeDeltaDTO`. The initial knowledge delta accumulated prior to executing the sequence.
        graph_context: `str`. A compact, read-only representation of the current graph context.
        entity_name_to_id: `dict[str, int] | None`. A mapping used to resolve existing entity names to their respective identifiers.
        entity_class_catalog: `dict[str, str] | None`. A catalog of known entity class definitions used for named entity recognition prompts.
        stage_names: `tuple[str, ...]`. An ordered tuple of model stage identifiers to be executed.
        event_callback: `LLMEventCallback | None`. An optional callback function for sinking live diagnostic events.
...
def merge_stage_delta(
    ...
    Args:
        ...
        accumulated_delta: `KnowledgeDeltaDTO`. The existing accumulated knowledge delta.
        stage_delta: `KnowledgeDeltaDTO`. The new knowledge delta generated by a specific LLM stage.
