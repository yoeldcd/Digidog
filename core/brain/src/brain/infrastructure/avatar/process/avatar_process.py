...
class AvatarProcessSupervisor:
    ...
    def pid(self) -> int | None:
        ...
        Returns:
            int | None: Live process identifier, or ``None`` when not running.
        """
    ...
    def ensure_running(self) -> int:
        ...
        Returns:
            int: Operating-system process identifier of the live avatar child.
        """
        live_pid = self.pid
    ...
    def close(self) -> None:
        ...
        Raises:
            subprocess.TimeoutExpired: If the child remains alive after both
                graceful termination and forced termination waits.
        """
        if self._process is None or self._process.poll() is not None:
            return
