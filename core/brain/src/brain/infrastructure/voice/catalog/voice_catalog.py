...
class VoiceCatalogService:
    ...
    def list_catalog(self, engine_name: str = "") -> dict[str, Any]:
        """Return a normalized catalog for one requested speech engine.

        Args:
            engine_name (str): Engine name, or an empty string for the active
                configured engine.

        Returns:
            dict[str, Any]: Engine mode, voices, models, settings, and warnings.

        Raises:
            ValueError: If the requested engine is unsupported.
            RuntimeError: If the selected engine cannot expose its catalog.
        """
        config = load_avatar_config()
        resolved_engine = (engine_name or config.active_voice_engine).strip().casefold()
        ...
        engine_config = getattr(config.voice_engines, resolved_engine).model_dump()
        resolver = getattr(self, f"_list_{resolved_engine}")
        catalog = resolver(engine_config)
        return {
            "engine": resolved_engine,
            "mode": ENGINE_MODES[resolved_engine],
            "active": resolved_engine == config.active_voice_engine.casefold(),
