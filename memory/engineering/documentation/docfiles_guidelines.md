...
1. Ensure the project contains `{BRAIN_SCRIPT_DIR}/documentation_utils/documentation_cli.js`. If missing, clone it from snippets `documentation_utils`.
2. Check documentation before generating: `node {BRAIN_SCRIPT_DIR}/documentation_utils/documentation_cli.js check <Project_PATH>/documentation`
3. Compile the documentation running: `node {BRAIN_SCRIPT_DIR}/documentation_utils/documentation_cli.js generate <Project_PATH>/documentation --log-domain <LOG_DOMAIN>`
   It generates a navigable live Markdown wiki on output dir `<Project_PATH>/documentation/wiki`
   Use log domain named associated to documented sources in log index: `py {LOCAL_BRAIN_SCRIPT} log-index --json`
4. Serve the generated wiki when live Markdown fetching is needed: `node {BRAIN_SCRIPT_DIR}/documentation_utils/documentation_cli.js serve <Project_PATH>/documentation`
